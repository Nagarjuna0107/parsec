...
def parse_reads(actions):
    ...
    for cmd in actions or []:
        ...
        if not files:
            continue
        rng = None
        m = re.search(r"\b(\d+),(\d+)p", cmd) or re.search(
            r"NR\s*>=\s*(\d+)\s*&&\s*NR\s*<=\s*(\d+)", cmd
        )
        if m:
...
def det_keep(messages, patch):
    ...
    chunks = sorted(
        accumulated_chunks(steps, T - 1, read_lines=CHUNK_LINES)
    ...
    def ctx(t):
        if t not in _ever:
            _ever[t] = (
                targets_after(steps, t, patch, None, precise_only=True),
    ...
    for age in (0, 1):
        for t in range(1 + age, T - 1):
            decided = [
                i for i, c in enumerate(chunks) if c.step == t - age and c.kind != "reasoning"
            ]
            if not decided:
                continue
            alive = [
                ...
                if c.step <= t and (c.step == t or (age == 1 and c.step == t - 1) or admitted[i])
            ]
            if len(alive) < 6:
                continue
...
def case_rare_token_trigger():
    ...
    msgs = PLAN + [
        asst("python -m pytest -x"),
        tool(
            "FAILED tests/test_q.py\nTypeError: zorblefrazzle_mode unsupported\n"
...
def case_asst_and_reasoning():
    ...
    msgs = pad(msgs, 3, "fillone")
    msgs += [
        asst("sed -n '1,5p' m.py && echo quuxplex_handler"),
        tool(lines("mcontent_line", 1, 5)),
    ]
    msgs = pad(msgs, 2, "filltwo")
...
def case_mixed_session():
    ...
    msgs += [
        asst("grep -rn gribblewock_state src/ tests/"),
