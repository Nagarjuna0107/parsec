...
_stub_module(
    "adaptive_context.optimizer.pyg_model", ["PyGCurator", "attach_blocks", "attach_task", "edges"]
)
_stub_module(
    "adaptive_context.optimizer.torch_curator",
...
ANALYSIS = "\n".join(
    f"Observation {i}: handler_{i % 12} looks {'fine' if i % 2 else 'wrong 🐍'}." for i in range(50)
)
...
LEAD_ASST_MESSAGES = [
    asst(
        ...
        cmd="cat src/app.py",
        reasoning="planning before the task arrives",
    ),
    {"role": "user", "content": "Fix the failing handler tests in src/app.py."},
...
# Override valve: an identical call restores an elided result, once; a user turn
# that inherits last_cmd is not the call's result; a narrower call is a new call.
OVERRIDE_MESSAGES = [
    {"role": "system", "content": "You are a coding agent."},
    {"role": "user", "content": "Fix the failing handler tests in src/app.py."},
    asst("Reading the file.", cmd="cat src/app.py"),
    {"role": "user", "content": PY_FILE},
    {"role": "user", "content": "Human follow-up while you read:\n" + ANALYSIS},
    asst("Same call again.", cmd="cat src/app.py"),
    {"role": "user", "content": PY_FILE},
    asst("A narrower call.", cmd="sed -n '10,30p' src/app.py"),
    {"role": "user", "content": SED_RANGE},
    asst("And the whole file once more.", cmd="cat src/app.py"),
    {"role": "user", "content": PY_FILE},
]
...
CONVERSATIONS = [
    ...
    {"name": "override", "messages": OVERRIDE_MESSAGES, "prefixes": [4, 5, 7, 9, 11]},
]
...
def registries(cur) -> dict:
    ...
    return {
        ...
        "dropped_cmds": sorted(cur.dropped_cmds),
    ...
    for label, tau_q in TAUS:
        ...
        for conv in CONVERSATIONS:
            ...
            partials = sum(
                ...
                and "[... ~" in m["content"]
                and "lines (~" not in m["content"]
            )
            stats.append(
                f"{conv['name']}: dropped={len(cur.dropped)} insists={cur.insists} "
