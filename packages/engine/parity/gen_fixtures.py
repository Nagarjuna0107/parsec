...
OUT = (
    sys.argv[2]
    if len(sys.argv) > 2
    else os.path.join(os.path.dirname(__file__), "fixtures", f"chunking_{MODE}.json")
)
...
CHUNK_CASES = [
    ...
    dict(
        name="other_test_output", cmd="python -m pytest -x", obs=TEST_OUT, step=6, read_lines=None
    ),
    ...
    dict(
        name="read_atoms_sed", cmd="sed -n '40,80p' src/big.py", obs=PY_SRC, step=3, read_lines=10
    ),
    ...
    dict(
        name="grep_search_beats_read", cmd="grep -n cat src/x.py", obs=GREP, step=1, read_lines=10
    ),
]
...
if MODE == "cst":  # CST only changes the read_lines code-read branch
    CHUNK_CASES += [
        ...
        dict(
            name="cst_too_small", cmd="cat t.py", obs="x = 1\ny = 2\nz = 3", step=1, read_lines=10
        ),
        dict(
            name="cst_flat_dump",
            cmd="cat dump.py",
...
MESSAGES = [
    ...
    {
        "role": "assistant",
        "content": [{"type": "text", "text": "Now read the file."}, {"text": "Part two."}],
        "extra": {"actions": [{"query": "open src/app.py"}, {"command": ""}]},
        "thinking_blocks": [{"thinking": "unicode 🐍 blob"}],
    },
    {"role": "user", "content": PY_SRC},
    {"role": "assistant", "content": [{"type": "image", "text": "nope"}], "extra": {"actions": []}},
...
def main():
    ...
    for a in ASSISTANT_CASES:
        fx["assistant_cases"].append(
            {**a, "expected": chunks_json(C.chunk_assistant(a["txt"], a["step"]))}
        )
    ...
    fx["dup_cases"].append(
        {
            "name": "dup_accum",
            "steps": [list(s) for s in dup_steps],
            "upto": 99,
            "read_lines": 10,
            "expected_sup": sup,
            "expected_dup_earlier": dup_earlier,
            "rows": rows,
            "expected_sup_rows": sup_r,
            "expected_dup_earlier_rows": dup_r,
        }
    )
