...
TEST_OUT = (
    "=== session ===\n"
    + "\n".join(f"test_mod.py::test_{i} PASSED" for i in range(1, 40))
...
def _dup_clamp():
    same = "def repeated():\n    return 42"
    out = [
        _mk(same, file="rep.py", lo=1, hi=2, step=s, kind="read", cmd=f"cat rep.py #{s}")
        for s in range(7)
    ]  # 6 earlier dups on the last -> min(1, 6/5) clamps
    out.append(
        _mk(
            "def repeated():",
            file="rep.py",
    ...
    )  # fractional sup on earlier rows
...
def _markers(chunks) -> list:
    ...
    return [
        [1.0 if c.kind == "reasoning" else 0.0, 1.0 if (getattr(c, "head", "") or "") else 0.0]
        for c in chunks
    ]
    ...
    fx = {
        ...
        "cases": [
            ...
            _case(
                "dup_heavy",
                dup_chunks,
                "cat src/app.py alpha method range total",
