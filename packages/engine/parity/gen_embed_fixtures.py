...
OUT = (
    sys.argv[1]
    if len(sys.argv) > 1
    else os.path.join(os.path.dirname(__file__), "fixtures", "hash_embed.json")
)

sys.path.insert(
    0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "brain", "src"))
)
...
def main() -> None:
    ...
    for text, dim in CASES:
        ...
        cases.append({"text": text, "dim": dim, "bits": [f32_bits(v) for v in vec]})
