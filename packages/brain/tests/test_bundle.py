...
def test_tolerates_training_only_cand_head(tmp_path):
    # v6+ trainer ckpts carry a cand_head (score_candidates, offline ranking) the serving
    # graph never calls; the loader must drop it by name rather than refuse the bundle.
    ck = torch.load(str(CKPT), map_location="cpu", weights_only=False)
    h = ck["hidden"]
    ck["state"] = dict(ck["state"], **{
        "cand_head.0.weight": torch.zeros(h, 2 * h + 10), "cand_head.0.bias": torch.zeros(h),
        "cand_head.3.weight": torch.zeros(1, h), "cand_head.3.bias": torch.zeros(1),
    })
    p = tmp_path / "with_cand_head.pt"
    torch.save(ck, str(p))
    b = load_bundle(ckpt_path=str(p))
    assert not hasattr(b.model, "cand_head")


def test_rejects_unknown_state_keys(tmp_path):
    ck = torch.load(str(CKPT), map_location="cpu", weights_only=False)
    ck["state"] = dict(ck["state"], **{"mystery_head.weight": torch.zeros(4, 4)})
    p = tmp_path / "unknown_head.pt"
    torch.save(ck, str(p))
    with pytest.raises(BundleError, match="did not consume"):
        load_bundle(ckpt_path=str(p))
