"""parsec-brain — the scoring service the local proxy calls for keep/cut scores.
Self-hostable: one checkpoint, one process; see README.md.

v0 (inference-first, docs/brain-serving-v0.md): serves a curator checkpoint behind
...
⚠️ The v0 dev contract (brain-api-dev/v0) carries RAW TEXT to the scoring host —
explicit client opt-in (PARSEC_BRAIN_DEV_RAW=1), for hosts the user operates. The same endpoints ALSO serve the
