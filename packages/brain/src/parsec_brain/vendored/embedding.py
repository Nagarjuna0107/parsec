...
Backends (PARSEC_EMBED_BACKEND):
  - "local":  the bge-large-en-v1.5 encoder loaded IN-PROCESS (local_embed.py) — no HTTP hop.
              The default and the recommended self-host shape (one host holds embedder + GNN).
  - "remote": the same bge-large-en-v1.5 encoder behind an HTTP embedding service you run
              (parsec_embed.py; PARSEC_EMBED_URL required). 1024-d, native batching.
  - "hash":   deterministic pseudo-embeddings (no network, NOT the trained vectors). Used by
              the unit tests and offline dev; scores from it are meaningless.
  (The reference's "vertex" branch is deleted in this vendored copy.)
