...
Loads the encoder the checkpoint was trained against (BAAI/bge-large-en-v1.5,
1024-d) INTO this process, so scoring is a single proxy -> brain -> proxy round
trip with no external embed service. This is the default and the recommended
self-host shape — one host (GPU or CPU) holds bge-large + the R-GCN.

Parity is the point: the checkpoint was trained on this encoder's vectors under
the recipe below, so any deployment must reproduce them. The recipe is pinned
to bge's reference usage (model card + SentenceTransformer config):
  ...
  * NO query instruction. The remote JSON protocol (parsec_embed.py) posts
...
Determinism note (matters for the round-robin-safe replica claim): CPU is
...
# HF id or a baked local dir (the image deploy sets PARSEC_EMBED_MODEL_DIR=/embed-model).
