...
_HF_PREFIX = "hf://"                                            # PARSEC_CKPT=hf://<repo_id>/<file>
_EXPECTED_RULES = 16
...
# Top-level module names of ckpt heads that exist only for training-time objectives.
# Kept as a closed set: extend deliberately when the trainer adds a head, never wildcard.
_TRAIN_ONLY_HEADS = frozenset({"cand_head"})
...
def _resolve_ckpt(spec: str) -> Path:
    """A local path, or `hf://<repo_id>/<filename>` fetched (and cached) from the Hugging
    Face Hub. The hub download is content-addressed by the hub's own etag; the sha256
    computed below is still the bundle's identity, so a swapped file never serves under
    an old checkpoint_id."""
    if not spec.startswith(_HF_PREFIX):
        return Path(spec).expanduser()
    ref = spec[len(_HF_PREFIX):]
    repo_id, sep, filename = ref.rpartition("/")
    if not sep or repo_id.count("/") != 1 or not filename:
        raise BundleError(f"PARSEC_CKPT={spec}: expected hf://<org>/<repo>/<filename>")
    try:
        from huggingface_hub import hf_hub_download
    except ImportError as e:  # pragma: no cover
        raise BundleError("PARSEC_CKPT=hf://… needs `pip install huggingface_hub`") from e
    log.info("fetching checkpoint from the Hugging Face Hub repo=%s file=%s", repo_id, filename)
...
def load_bundle(ckpt_path: str | None = None, rules_path: str | None = None,
    ...
    ckpt_file = _resolve_ckpt(ckpt_path or os.environ.get("PARSEC_CKPT", _DEFAULT_CKPT))
    ...
    # Training-only heads the serving graph never calls. The trainer's PyGCurator (v6+)
    # grows a cand_head for score_candidates() — offline candidate ranking, not on the
    # /v1/score path — so its weights are dropped here, explicitly, by prefix. Anything
    # else unconsumed is still a hard error: an unknown key means an unknown architecture.
    train_only = [k for k in unexpected if k.split(".", 1)[0] in _TRAIN_ONLY_HEADS]
    if train_only:
        log.info("dropping training-only head params not served: %s", train_only)
    unexpected = [k for k in unexpected if k not in train_only]
    if unexpected:
