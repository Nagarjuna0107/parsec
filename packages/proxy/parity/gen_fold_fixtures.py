...
OUT = (
    sys.argv[1]
    if len(sys.argv) > 1
    else os.path.join(os.path.dirname(__file__), "fixtures", "fold.json")
)
    ...
    (
        ...
        [
            {"type": "text", "text": "t", "cache_control": {"type": "ephemeral"}},
            {
                ...
                "content": [
                    {"type": "text", "text": "inner", "cache_control": {"type": "ephemeral"}}
                ],
            },
        ],
    ),
        ...
        [
            {
                ...
                "input": {
                    ...
                    "c": 1e16,
                    "d": -0.0,
                    "e": 1.1534175185142759,
                    "f": 123456789,
                    "g": [1.0, 0.5, 1e15, -2.5e-7],
                    "h": 0.0001,
                },
            }
        ],
...
def main():
    ...
    for name, c in CONTENTS:
        ...
        fx["strip_cache_control"].append(
            {"name": name, "content": c, "expected": S._strip_cache_control(c)}
        )
        fx["fingerprint"].append({"name": name, "content": c, "expected": S._orig_fingerprint(c)})
        for t in NEW_TEXTS:
            fx["rewrite_first_text"].append(
                {
                    "name": f"{name}|{t[:8]}",
                    "content": c,
                    "new_text": t,
    ...
    fx["apply_curation"].append(
        {
            "name": "stateless_trim",
            "body": b1,
            "curated": curated(cur_texts),
            "state": None,
            "expected": S.apply_curation(b1, curated(cur_texts), None),
            "expected_state": None,
        }
    )
    ...
    fx["apply_curation"].append(
        {
            "name": "stateless_unchanged",
            "body": b1,
            "curated": curated(same_texts),
            "state": None,
            "expected": S.apply_curation(b1, curated(same_texts), None),
            "expected_state": None,
        }
    )
    ...
    fx["apply_curation"].append(
        {
            "name": "stateless_short_curated",
            "body": b1,
            "curated": curated(cur_texts[:2]),
            "state": None,
            "expected": S.apply_curation(b1, curated(cur_texts[:2]), None),
            "expected_state": None,
        }
    )
    # no system
    ...
    fx["apply_curation"].append(
        {
            "name": "stateless_no_system",
            "body": b2,
            "curated": cur2,
            "state": None,
            "expected": S.apply_curation(b2, cur2, None),
            "expected_state": None,
        }
    )
    ...
    r2 = S.apply_curation(
        b3, curated(["SYS DIFFERENT", "task NEW", "plan NEW", "obs NEW", "next step"]), state2
    )
    fx["apply_curation"].append(
        {
            "name": "freeze_call2_replays",
            "body": b3,
    ...
    fx["apply_curation"].append(
        {
            "name": "freeze_call3_refold_changed_turn",
            "body": b4,
            "curated": cur_changed + [curated(["next step"])[0]],
            "state": json.loads(json.dumps(state2)),
            "expected": r3,
