...
OUT = (
    sys.argv[1]
    if len(sys.argv) > 1
    else os.path.join(os.path.dirname(__file__), "fixtures", "internal.json")
)
...
BODIES = [
    ...
    (
        "tool_result_empty_parts",
        B([{"role": "user", "content": [TR_EMPTY_PARTS, {"type": "text", "text": "tail"}]}]),
    ),
    ("parallel_tool_results", B([{"role": "user", "content": [TR, TR_EMPTY_PARTS, TRE]}])),
    (
        "thinking_blocks_skipped",
        B(
            [
                asst(
    ...
    (
        "non_dict_parts",
        B([{"role": "user", "content": [{"type": "text", "text": "x"}, "bare", 7]}]),
    ),
    ...
    (
        "system_non_list_dict",
        B([{"role": "user", "content": "hi"}], system={"type": "text", "text": "x"}),
    ),
    ...
    (
        "user_tool_use_ignored",
        B(
            [
                {
                    "role": "user",
                    "content": [tu("Bash", {"command": "ls"}), {"type": "text", "text": "u"}],
                }
            ]
        ),
    ),
    ...
    (
        ...
        B(
            [
                asst(
                    tu("BashOutput", {"command": "tail -f log"}, id="tu_a"),
    ...
    (
        "read_offset_limit",
        B([asst(tu("Read", {"file_path": "/a/b.py", "offset": 10, "limit": 5}))]),
    ),
    ...
    (
        "read_null_offset",
        B([asst(tu("Read", {"file_path": "/a/b.py", "offset": None, "limit": 3}))]),
    ),
    (
        "read_str_nums",
        B([asst(tu("Read", {"file_path": "/a/b.py", "offset": "12", "limit": " 3 "}))]),
    ),
    (
        "read_invalid_offset_nulls_both",
        B([asst(tu("Read", {"file_path": "/a/b.py", "offset": "abc", "limit": 5}))]),
    ),
    (
        "read_invalid_limit_nulls_both",
        B([asst(tu("Read", {"file_path": "/a/b.py", "offset": 10, "limit": "x"}))]),
    ),
    (
        "read_float_bool",
        B([asst(tu("Read", {"file_path": "/a/b.py", "offset": 5.9, "limit": True}))]),
    ),
    (
        "read_negative_limit",
        B([asst(tu("Read", {"file_path": "/a/b.py", "offset": 7, "limit": -3}))]),
    ),
    (
        "read_name_variants",
        B(
    ...
    (
        "read_file_path_falsy_falls_through",
        B([asst(tu("Read", {"file_path": "", "path": "/real"}))]),
    ),
    ("read_file_nonstr_derives", B([asst(tu("Read", {"file_path": [1], "path": "/real"}))])),
    # ── edit twin ──
    (
        "edit_basename",
        B([asst(tu("Edit", {"file_path": "/a/b/c.py", "old_string": "x", "new_string": "y"}))]),
    ),
    ...
    (
        "multiedit",
        B([asst(tu("MultiEdit", {"file_path": "src/main.rs", "edits": [{"old": "a"}]}))]),
    ),
    # ── grep twin ──
    ...
    (
        "unknown_tool_total_clip",
        B([asst(tu("WebFetch", {"a": "p" * 200, "b": "q" * 200, "c": "r" * 200}))]),
    ),
    ("unknown_tool_empty_args", B([asst(tu("TodoWrite", {}))])),
    # ── splat ordering ──
    (
        "splat_overrides_tool_name_command",
        B(
            [
                asst(
                    tu(
    ...
    (
        ...
        B(
            [
                ...
                asst(
                    tu("Grep", {"pattern": "def x", "path": "src"}, id="tu_b"),
                    tu(
                        "Edit",
...
def main():
    ...
    for name, tool, args in TWIN_CASES:
        fx["bash_twin_command"].append(
            {"name": name, "tool": tool, "args": args, "expected": bash_twin_command(tool, args)}
        )
    for name, tool, args in DCMD_CASES:
        fx["derive_command"].append(
            {"name": name, "tool": tool, "args": args, "expected": derive_command(tool, args)}
        )
    for name, tool, args in DQ_CASES:
        fx["derive_query"].append(
            {"name": name, "tool": tool, "args": args, "expected": derive_query(tool, args)}
        )
