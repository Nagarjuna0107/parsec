...
/// The dashboard origin `parsec login` opens (`/connect`) and accepts the
/// key handoff from. `PARSEC_DASHBOARD_URL` overrides for a dev dashboard on
/// localhost; release behaviour is [`SIGNUP_URL`]. Trailing slash trimmed so
/// it composes with paths and compares equal to a browser `Origin` header.
pub fn dashboard_url() -> String {
    std::env::var("PARSEC_DASHBOARD_URL")
        .ok()
