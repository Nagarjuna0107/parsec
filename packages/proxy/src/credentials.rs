...
//! ships savings-ledger rows with — written once by `parsec login` (the browser
//! handoff, `login.rs`; also what the installers, the tray and the
//! `/parsec:login` skill run) or by hand with `parsec key set` on a browserless
//! machine, instead of editing env vars or settings.json. Read by
//! `ledger_ship::resolve` at ship time (no proxy restart needed — it re-reads
//! the file per shipped row).
//!
