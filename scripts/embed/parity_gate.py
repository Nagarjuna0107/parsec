...
def onnx_embed(model_dir: Path, model_file: str, texts: list[str]) -> np.ndarray:
    ...
    sess = rt.InferenceSession(str(model_dir / model_file), providers=["CPUExecutionProvider"])
...
def remote_embed(url: str, texts: list[str]) -> np.ndarray | None:
    ...
    try:
        ...
        resp = requests.post(url, json={"model_id": MODEL_ID, "texts": texts}, timeout=360)
...
def compare(
    name: str, ref: np.ndarray, got: np.ndarray, texts: list[str], threshold: float | None
) -> bool:
...
def main() -> int:
    ...
    ap.add_argument(
        "--remote-url", default=os.environ.get("PARSEC_EMBED_URL", "http://127.0.0.1:18080/embed")
    )
    ap.add_argument("--skip-remote", action="store_true")
    ap.add_argument(
        "--int8", action="store_true", help="also compare model.int8.onnx (informational)"
    )
    ap.add_argument(
        "--battery-out", type=Path, help="write the battery texts as JSON (for onnx_battery.rs)"
    )
    ap.add_argument(
        "--rust-vectors", type=Path, help='{"vectors": ...} from the onnx_battery example'
    )
    args = ap.parse_args()
    ...
    if not args.skip_remote:
        ...
        if remote is None:
            ...
            ok &= compare("remote service vs local fp32 onnx", ref, remote, texts, PASS_COSINE)

    if args.rust_vectors:
        rust = np.asarray(json.loads(args.rust_vectors.read_text())["vectors"], dtype=np.float32)
        ok &= compare(
            "rust OnnxEmbedder vs python onnxruntime (same export)", ref, rust, texts, PASS_COSINE
        )

    if args.int8:
        int8 = onnx_embed(args.model_dir, "model.int8.onnx", texts)
        compare("int8 dynamic-quant vs fp32 (informational)", ref, int8, texts, None)
