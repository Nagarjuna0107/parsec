...
def sanitize_body(body: dict) -> dict:
    ...
    if isinstance(out.get("system"), str):
        ...
        out["system"] = [_sanitize_block(b) if isinstance(b, dict) else b for b in out["system"]]
    if isinstance(out.get("messages"), list):
        out["messages"] = [
            {**m, "content": _sanitize_content(m.get("content"))} if isinstance(m, dict) else m
            for m in out["messages"]
        ]
    if isinstance(out.get("metadata"), dict):
        out["metadata"] = {
            k: scramble_text(v) if isinstance(v, str) else v for k, v in out["metadata"].items()
        }
    return out
...
def tool_schema(name: str) -> dict:
    desc, props, req = TOOL_STUBS.get(name, ("A tool available in this session.", {}, []))
...
def load_transcript(path: pathlib.Path) -> list[dict]:
    ...
    for line in path.open():
        ...
        mid = m.get("id")
        if merged and merged[-1][0] == role and (role == "user" or (mid and merged[-1][2] == mid)):
...
def turns_from_record_dir(path: pathlib.Path, max_turns: int) -> list[dict]:
    files = sorted(
        (f for f in path.iterdir() if re.fullmatch(r"turn_\d+\.json", f.name)),
