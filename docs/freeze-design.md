...
- birth admission: override valve (`_insisted`: an identical call — same
  `cmd` — whose earlier observation had a chunk elided is served in full,
  once; `dropped_cmds` registry), `_budget_cut` (score-ascending stable sort, stop at
