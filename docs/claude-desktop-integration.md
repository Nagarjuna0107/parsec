...
The native `.pkg` (`packages/installer/macos/`) folds both passes into one
flow. Its desktop postinstall runs `parsec setup desktop --prepare
--autostart` as the user — everything up to the gate, no `sudo`, no
interceptor start, exit 0 — which also registers the extension
(`request_extension_registration`: a throwaway local-mode `mitmdump` run,
polled until `systemextensionsctl` lists it), opens the System Settings pane,
and leaves `~/.parsec/desktop-approval-pending.json`. The CA is then trusted
by the root postinstall directly (Installer.app already authenticated). The
tray, installed by the same pkg, reads the marker on launch and starts its
guided wait with no click; when the toggle flips it runs `parsec setup
desktop [--autostart]`, i.e. `provision()` — the half of `run_start` past the
gate. A machine whose extension is already approved skips all of that: the
postinstall sees `"extension":"ready"` in the `PARSEC_DESKTOP_PREPARE` report
line and provisions immediately.
...
7. Spawn `mitmdump` detached (own process group / no console), PID at
   ...
   Windows autostart is a **Scheduled Task** (`ParsecInterceptor`, `/SC
   ONLOGON /RL HIGHEST`, launching a hidden-window `.vbs` → the `.cmd`),
   not a Run-key value: the Run key fires *unelevated* at sign-in, WinDivert
   fails to load, and nothing is intercepted after a reboot — which is how
   the first Windows autostart silently did nothing. Registering a
   highest-privilege task needs administrator once, which is why
   `install_service` bails with the elevated re-run command when it lacks
   it; the native installer's one UAC prompt covers it. The legacy Run
   value is recognised (`service_installed`, `status`) and deleted on the
   next `--autostart`, so pre-installer machines migrate by re-running
   `parsec setup desktop --autostart` from an administrator PowerShell.
   Open QA item: confirm `schtasks /SC ONLOGON` without `/RU` registers the
   trigger for the creating user with the interactive token; if it registers
   for any user, add `/RU "%USERDOMAIN%\%USERNAME%" /IT`.
...
- **Windows, the native installer** (`packages/installer/windows/parsec.iss`):
  per-user Inno Setup, one `ShellExec('runas', parsec.exe, 'setup desktop
  --install-ca --autostart')` for the whole Desktop provision — WinDivert,
  the machine root-store CA, and the scheduled task in one UAC prompt,
  positioned after every user-scope step so a declined prompt costs nothing
  else. `parsec` itself still never self-elevates.
